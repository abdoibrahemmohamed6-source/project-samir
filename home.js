...
function showstudent() {
 ...
 cards.innerHTML +=
                ...
                <h2 class="py-2 text-center">${student.name}</h2>
                <p>Level: ${student.level} sec</p>
                <span class="fs-4 fw-bold text-center py-2">L.E${student.price}</span>
                 <svg
...
function showAllStudent(){
    ...
    students.forEach((student) => {
        totalPrice += Number(student.price);
    });
    
    total.innerHTML = `<span class="fs-4 fw-bold text-center py-2">L.E ${totalPrice}</span>
      <button type="button" onclick="showAllStudent()" class="btn btn-primary text-uppercase">Total</button>
    `;
}
...
function editstudent(index) {
  ...
  priceinput.value = students[index].price;
